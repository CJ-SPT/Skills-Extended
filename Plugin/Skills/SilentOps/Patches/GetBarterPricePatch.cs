...
public class GetBarterPricePatch : ModulePatch
{
    ...
    protected override MethodBase GetTargetMethod()
    {
        return AccessTools.Method(
            typeof(TraderAssortmentControllerClass),
    ...
    private static void Postfix(
        TraderAssortmentControllerClass __instance,
        ref TraderClass.GStruct300? __result,
        Item[] items
    )
    {
        ...
        foreach (var item in items)
        {
            ...
            var num2 = Mathf.Ceil(
                (float)
                    barterScheme.Sum(TraderAssortmentControllerClass.Class2058.class2058_0.method_0)
            );
            var bonus =
                1f - GameUtils.GetSkillManager()!.SkillManagerExtended.SilentOpsSilencerCostRedBuff;
...
public class RequiredItemsCountPatch : ModulePatch
{
    ...
    private static void Postfix(GClass2064 __instance, ref int __result)
    {
        if (
            ...
            || GetBarterPricePatch.Selecteditem is not SilencerItemClass
        )
        {
            return;
        }
