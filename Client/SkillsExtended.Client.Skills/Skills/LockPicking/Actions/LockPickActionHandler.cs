...
public sealed class LockPickActionHandler
{
    ...
    {
        var data = new LockPickingEventData { DoorId = InteractiveObject.Id };
    ...
    private void RemoveUseFromLockPick()
    {
        ...
        if (skillManager?.SkillsExtendedManager.LockPickingUseBuffElite.Value ?? false)
        {
            return;
        }
        ...
        if (
            pick.KeyComponent.NumberOfUsages >= pick.KeyComponent.Template.MaximumNumberOfUsage
            && pick.KeyComponent.Template.MaximumNumberOfUsage > 0
        )
        {
            var result = ItemManipulator.Discard(pick, (ItemController)pick.Parent.GetOwner());
            if (result.Failed)
            {
