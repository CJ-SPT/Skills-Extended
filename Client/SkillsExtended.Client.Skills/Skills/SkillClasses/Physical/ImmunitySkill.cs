    ...
    )
{
    private static SkillManager.SkillAction[] GetActions(SkillManager skillManager)
    {
        ...
        [
            skillManager
                .HealthNegativeEffect.Where(effect => effect is IIntoxication)
                .Factor(skillManager.Settings.Immunity.HealthNegativeEffect),
            skillManager.StimulatorNegativeBuff.Factor(
                skillManager.Settings.Immunity.StimulatorNegativeBuff
            ),
        ];
    }
    ...
    private static SkillManager.Buff[] GetBuffs(SkillManager skillManager)
    {
        ...
        return
        [
            skillManager.ImmunityMiscEffects.PerLevel(
                data.MiscEffectsPerLevel.NormalizeToPercentage()
            ),
            skillManager.ImmunityPoisonBuff.PerLevel(
                data.PoisonBuffPerLevel.NormalizeToPercentage()
            ),
            skillManager.ImmunityPainKiller.PerLevel(
                data.PainKillerPerLevel.NormalizeToPercentage()
            ),
            skillManager
                .ImmunityAvoidPoisonChance.Default(0f)
                .Elite(data.AvoidPoisonChanceElite.NormalizeToPercentage()),
            skillManager
                .ImmunityAvoidMiscEffectsChance.Default(0f)
