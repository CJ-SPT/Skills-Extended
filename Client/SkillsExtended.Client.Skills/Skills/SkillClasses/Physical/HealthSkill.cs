    ...
    )
{
    ...
    private static SkillManager.SkillAction[] GetActions(SkillManager skillManager)
    {
        ...
        [
            skillManager
                .SkillProgress.Where(IsSkillRelatedToHealth)
    ...
    private static SkillManager.Buff[] GetBuffs(SkillManager skillManager)
    {
        ...
        return
        [
            skillManager.HealthBreakChanceRed.PerLevel(
                data.BreakChanceRedPerLevel.NormalizeToPercentage()
            ),
            skillManager.HealthEnergy.PerLevel(data.EnergyPerLevel.NormalizeToPercentage()),
            skillManager.HealthHydration.PerLevel(data.HydrationPerLevel.NormalizeToPercentage()),
            skillManager.HealthEliteAbsorbDamage,
        ];
    }

    private static bool IsSkillRelatedToHealth(Skill skill)
    {
