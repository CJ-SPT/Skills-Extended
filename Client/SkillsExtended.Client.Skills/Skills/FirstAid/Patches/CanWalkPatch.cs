...
internal class CanWalkPatch : ModulePatch
{
    ...
    [PatchPostfix]
    public static void PostFix(MovementContext __instance, Player ____player, ref bool __result)
    {
        ...
        var skillMgrExt = ____player.Skills.SkillsExtendedManager;
        var skillData = SkillsExtendedPlugin.SkillData.FirstAid;

        if (!skillData.Enabled)
            return;
        if (!skillMgrExt.FirstAidMovementSpeedBuffElite)
            return;
