...
internal class OnGameStartedPatch : ModulePatch
{
    ...
    {
        var healthControllerType = PatchConstants.EftTypes.Single(t =>
            t.Name is nameof(ActiveHealthController)
        );
        var nestedTypes = healthControllerType.GetNestedTypes(
    ...
    private static void ApplyMedicalXp(IHealthEffect effect)
    {
        var skillMgrExt = Player!.Skills.SkillsExtendedManager;

        if (
            SkillsExtendedPlugin.SkillData.FieldMedicine.Enabled
                && _stimType.IsInstanceOfType(effect)
            || _painKillerType.IsInstanceOfType(effect)
        )
        {
        ...
        if (
            ...
            && _medEffectType.IsInstanceOfType(effect)
        )
        {
    ...
    private static void ApplyNatoRifleXp(Mastering mastering)
    {
    ...
    private static void ApplyEasternRifleXp(Mastering mastering)
    {
    ...
    {
        foreach (
            var interactableObj in LocationScene.GetAllObjectsAndWhenISayAllIActuallyMeanIt<WorldInteractiveObject>()
        )
        {
            ...
            var logMessage = SkillsExtendedPlugin.Keys.KeyLocale.TryGetValue(
                interactableObj.KeyId,
                out var name
            )
